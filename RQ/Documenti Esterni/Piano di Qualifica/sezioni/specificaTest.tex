...
\begin{center}
    ...
    \begin{longtable}[!h]{p{45px} p{255px} p{35px} p{35px}}
        ...
        TU13          & Verifica che venga visualizzato lo stato d'errore nel caso in cui l'accesso alla funzioine `createProduct` si negato all'utente.                                                                              & I              & S              \\
        TU14          & Verifica che la funzione `deleteProduct` elimini correttamente l'oggetto desiderato                                                                                                              & I              & S              \\
        TU15          & Veridica che se la chiamata alla funzione `deleteProduct` venga effettuata su un oggetto inesistente la funzione ritorna il corretto errore di non esistenza del prodotto cercato                & I              & S              \\
        ...
        TU20          & Verifica che se la chiamata viene effettuata su un oggetto inesistente la funzione `getProduct` ritorni il corretto errore di non esistenza del prodotto richiesto                               & I              & S              \\
        ...
        TU24          & Verifica che nel caso in cui il formato del filtro non sia conforme l'handler di `scanProduct` ritorna il corretto il messaggio di errore                                                        & I              & S              \\
        ...
        TU28          & Verifica che l'handler di `updateProduct` controlli correttamente se l'utente sia autorizzato a chiamare la funzione `createProduct`. In caso positivo effettua la chiamata e genera il prodotto & I              & S              \\
        TU29          & Verifica che venga visualizzato lo stato d'errore nel caso in cui l'accesso alla funzione `updateProduct` si negato all'utente.                                                                              & I              & S              \\
        TU30          & Verifica che la funzione `createOrder` crei l'ordine correttamente                                                                                                                               & I              & S              \\
        TU31          & Verifica che l'handler di `createOrder` passi i corretti parametri alla funzione `createOrder`                                                                                                   & I              & S              \\
        TU32          & Verifica che l'handler di `createOrder` controlli che l'utente sia autorizzato a chiamare la funzione `createOrder`                                                                              & I              & S              \\
        TU33          & Verifica che se l'utente non è autorizzato ad utilizzare la funzione l'handler di `createOrder` ritorni il corretto messaggio di errore                                                        & I              & S              \\
        TU34          & Verifica che la funzione `deleteOrder` rimuova l'ordine correttamente                                                                                                                            & I              & S              \\
        TU35          & Verifica che l'handler di `deleteOrder` passi i corretti parametri alla funzione `deleteOrder`                                                                                                   & I              & S              \\
        TU36          & Verifica che l'handler di `deleteOrder` controlli che l'utente sia autorizzato a chiamare la funzione `deleteOrder`                                                                              & I              & S              \\
        TU37          & Verifica che se l'utente non è autorizzato ad utilizzare la funzione l'handler di `deleteOrder` ritorni il corretto messaggio di errore                                                        & I              & S              \\
        TU38          & Verifica che la funzione `getOrder` ritorni l'ordine atteso                                                                                                                                      & I              & S              \\
        TU39          & Verifica che la funzione `createTag` crei il tag correttamente                                                                                                                                   & I              & S              \\
        TU40          & Verifica che l'handler di `createTag` passi correttamente i parametri alla funzione `createTag`                                                                                                  & I              & S              \\
        TU41          & Verifica che l'handler di `createTag` controlli che l'utente sia autorizzato a chiamare la funzione `createTag`                                                                                  & I              & S              \\
        TU42          & Verifica che se l'utente non è autorizzato ad utilizzare la funzione l'handler di `createTag` ritorni il corretto messaggio di errore                                                          & I              & S              \\
        TU43          & Verifica che la funzione `deleteTag` rimuova il tag correttamente                                                                                                                                & I              & S              \\
        TU44          & Verifica che l'handler di `deleteTag` rimuova il tag correttamente                                                                                                                               & I              & S              \\
        TU45          & Verifica che l'handler di `deleteTag` controlli che l'utente sia autorizzato a chiamare la funzione `deleteTag`                                                                                  & I              & S              \\
        TU46          & Verifica che se l'utente non è autorizzato ad utilizzare la funzione l'handler di `deleteTag` ritorni il corretto messaggio di errore                                                          & I              & S              \\
        TU47          & Verifica che la funzione `getTag` ritorni il tag atteso                                                                                                                                          & I              & S              \\
        TU48          & Verifica che se il tag richiesto non esiste la funzione `getTag` ritorna il corretto messaggio di errore                                                                                         & I              & S              \\
        TU49          & Verifica che l'handler di `getTag` ritorni il tag atteso                                                                                                                                         & I              & S              \\
        TU50          & Verifica che l'handler di `getTag` controlli che l'utente sia autorizzato a chiamare la funzione `getTag`                                                                                        & I              & S              \\
        TU51          & Verifica che se l'utente non è autorizzato ad utilizzare la funzione l'handler di `getTag` ritorni il corretto messaggio di errore                                                             & I              & S              \\
        TU52          & Verifica che la funzione `updateTag` non sollevi eccezioni                                                                                                                                       & I              & S              \\
        TU53          & Verifica che l'handler di `updateTag` modifichi un tag                                                                                                                                           & I              & S              \\
        TU54          & Verifica che l'handler di `updateTag` controlli che l'utente sia autorizzato a chiamare la funzione `updateTag`                                                                                  & I              & S              \\
        TU55          & Verifica che se l'utente non è autorizzato ad utilizzare la funzione l'handler di `updateTag` ritorni il corretto messaggio di errore                                                          & I              & S              \\
        TU56          & Verifica che la funzione di `scanTag` applichi i limiti correttamente                                                                                                                            & I              & S              \\
        TU57          & Verifica che l'handler di `scanTag` applichi i limiti correttamente                                                                                                                              & I              & S              \\
        TU58          & Verifica che l'handler di `scanTag` controlli che l'utente sia autorizzato a chiamare la funzione `scanTag`                                                                                      & I              & S              \\
        TU59          & Verifica che se l'utente non è autorizzato ad utilizzare la funzione l'handler di `scanTag` ritorni il corretto messaggio di errore                                                            & I              & S              \\
    \end{longtable}
\end{center}
